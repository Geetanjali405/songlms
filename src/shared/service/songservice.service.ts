...
export class SongserviceService {
  ...
  addSong(song: song): boolean {
    if (this.songData.find(s => s.id === song.id)) {
      return false;
    }
  ...
  filterSongsBySongNameAndArtistName(songName: string, artistName: string): song[] {
    return this.songData.filter(song => song?.artistName?.toLowerCase()?.includes(artistName?.toLowerCase()) && song?.songName?.toLowerCase()?.includes(songName?.toLowerCase()));
  }
